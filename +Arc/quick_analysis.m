function percent_correct = quick_analysis(data)
...
unique_change_location = unique(change_location);
...
for cert_index = 1:length(unique_cue_certainty)
    ...
    for loc_index = 1:length(unique_change_location)
        ...
        current_invalid_trials = current_pct & current_location & valid_trials == 0;
        ...
        percent_correct.invalid(cert_index, loc_index) = mean(data.trialCorrect(current_invalid_trials),'omitmissing');
        percent_correct.invalid_n(cert_index, loc_index) = sum(isfinite(data.trialCorrect(current_invalid_trials)));
        tic

    end

figure(1), plot(unique_change_location,percent_correct.valid(cert_index,:),'-o')
figure(2), plot(unique_change_location,percent_correct.invalid(cert_index,:),'-p')
end
...
figure(3),clf
hold on
plot(unique_change_location,percent_correct.valid(2,:),'-or')
plot(unique_change_location,percent_correct.invalid(2,:),'-ob')
title('75% valid')
xlabel('spatial location')
ylabel('% correct')
...
figure(4),clf
hold on
plot(unique_change_location,percent_correct.valid(3,:),'-or')
plot(unique_change_location,percent_correct.valid(1,:),'-ob')
